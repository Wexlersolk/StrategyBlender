...
input string InpExitModel = "time_exit";

input group "Parameters"
input int HighestPeriod = 36;
...
void CancelExpiredOrders(const datetime bar_time)
{
   for(int idx = OrdersTotal() - 1; idx >= 0; --idx)
   {
      ...
      if((type == ORDER_TYPE_BUY_STOP || type == ORDER_TYPE_BUY_LIMIT) && comment == "XAU_breakout_04_long" && g_long_expiry != 0 && bar_time >= g_long_expiry)
         trade.OrderDelete(ticket);
      if((type == ORDER_TYPE_SELL_STOP || type == ORDER_TYPE_SELL_LIMIT) && comment == "XAU_breakout_04_short" && g_short_expiry != 0 && bar_time >= g_short_expiry)
         trade.OrderDelete(ticket);
   }
}
...
void OnTick()
{
   ...
   if(InpEntryArchetype == "ema_reclaim" || InpEntryArchetype == "pullback_trend" || InpEntryArchetype == "breakout_close")
   {
      if(LongSignalMarket(atr_value))
      {
         ...
         trade.Buy(InpLots, _Symbol, 0.0, sl, tp, "XAU_breakout_04_long");
         return;
      }
      if(ShortSignalMarket(atr_value))
      {
         ...
         trade.Sell(InpLots, _Symbol, 0.0, sl, tp, "XAU_breakout_04_short");
         return;
      }
      return;
   }
   ...
   {
      if(order_type == ORDER_TYPE_BUY_STOP && trade.BuyStop(InpLots, entry_price, _Symbol, sl, tp, ORDER_TIME_SPECIFIED, expiry, "XAU_breakout_04_long"))
         g_long_expiry = expiry;
      if(order_type == ORDER_TYPE_BUY_LIMIT && trade.BuyLimit(InpLots, entry_price, _Symbol, sl, tp, ORDER_TIME_SPECIFIED, expiry, "XAU_breakout_04_long"))
         g_long_expiry = expiry;
      return;
   }
   ...
   {
      if(order_type == ORDER_TYPE_SELL_STOP && trade.SellStop(InpLots, entry_price, _Symbol, sl, tp, ORDER_TIME_SPECIFIED, expiry, "XAU_breakout_04_short"))
         g_short_expiry = expiry;
      if(order_type == ORDER_TYPE_SELL_LIMIT && trade.SellLimit(InpLots, entry_price, _Symbol, sl, tp, ORDER_TIME_SPECIFIED, expiry, "XAU_breakout_04_short"))
         g_short_expiry = expiry;
   }
}
